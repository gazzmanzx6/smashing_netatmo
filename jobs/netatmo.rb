...
SCHEDULER.every interval, :first_in => 0 do
  tokens = JSON.parse(File.read('/home/pi/smashing-dashboards/dashboard/tokens.json'))
  access_token = tokens['access_token']
  
  answer = JSON.parse(Curl.get("https://api.netatmo.net/api/getstationsdata?access_token=#{access_token}").body_str)
